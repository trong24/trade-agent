...
def _make_df(
    count: int, start_ms: int = 1_700_000_000_000, interval_ms: int = 900_000
) -> pd.DataFrame:
...
def test_no_gaps_perfect_data():
    df = _make_df(100)
    report = validate(df, "BTCUSDT", "15m")
...
def test_detect_gap():
    ...
    report = validate(df, "BTCUSDT", "15m", gap_threshold=5)
...
def test_detect_duplicates():
    ...
    report = validate(df_with_dup, "BTCUSDT", "15m")
